...
class TestConfigLoading(unittest.TestCase):
    ...
    def setUp(self):
        self.default_config = {"model": "DQN", "device": "cpu", "erase": False}
        self.user_config = {"model": "PPO", "learning_rate": 0.001}

    @patch("os.path.exists")
    @patch("builtins.open", new_callable=mock_open)
    def test_load_default_config(self, mock_file, mock_exists):
        mock_exists.return_value = True
        mock_file.return_value.__enter__().read.return_value = json.dumps(
            self.default_config
        )
