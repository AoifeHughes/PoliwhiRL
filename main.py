...
def parse_args():
    ...
    parser.add_argument("--rom_path", type=str, default=config.get("rom_path", "./emu_files/Pokemon - Crystal Version.gbc"))
    parser.add_argument("--state_path", type=str, default=config.get("state_path", "./emu_files/states/start.state"))
    parser.add_argument("--episode_length", type=int, default=config.get("episode_length", 25))
    parser.add_argument("--device", type=str, default=config.get("device", "cpu"))
    parser.add_argument("--num_episodes", type=int, default=config.get("num_episodes", 10000))
    parser.add_argument("--batch_size", type=int, default=config.get("batch_size", 32))
    parser.add_argument("--checkpoint", type=str, default=config.get("checkpoint", "./checkpoints/RainbowDQN.pth"))
    parser.add_argument("--model", type=str, default=config.get("model", "RainbowDQN"))
    parser.add_argument("--sight", action="store_true", default=config.get("sight", False))
    parser.add_argument("--erase", action="store_true", default=config.get("erase", False))
    parser.add_argument("--parallel", action="store_true", default=config.get("parallel", False))
    parser.add_argument("--runs_per_worker", type=int, default=config.get("runs_per_worker", 4))
