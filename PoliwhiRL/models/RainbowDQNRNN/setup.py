...
from .sequencedprioritizedreplaybuffer import SequencedPrioritizedReplayBuffer
from .utils import save_checkpoint, load_checkpoint
from .rrnrainbow import run as run_single
...
def initialize_training(config, env):
    ...
    policy_net = LSTMDQN(input_shape, len(env.action_space), config["device"]).to(
        config["device"]
    )
    target_net = LSTMDQN(input_shape, len(env.action_space), config["device"]).to(
        config["device"]
    )
    optimizer = optim.Adam(policy_net.parameters(), lr=config["learning_rate"])
    replay_buffer = SequencedPrioritizedReplayBuffer(config["capacity"], config["device"], config["alpha"])
