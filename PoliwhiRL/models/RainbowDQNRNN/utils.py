...
def store_experience(state, action, reward, next_state, done, policy_net, target_net, replay_buffer, config):
    action_tensor = torch.tensor([action], dtype=torch.long, device=config["device"])
    reward_tensor = torch.tensor([reward], dtype=torch.float, device=config["device"])
    done_tensor = torch.tensor([done], dtype=torch.bool, device=config["device"])

    # Calculate TD error for the given experience
    td_error = compute_td_error(
        state.unsqueeze(0).unsqueeze(0),  # Add batch and sequence dimension
        action_tensor.unsqueeze(0).unsqueeze(0),  # Add batch and sequence dimension
        reward_tensor.unsqueeze(0).unsqueeze(0),  # Add batch and sequence dimension
        next_state.unsqueeze(0).unsqueeze(0),  # Add batch and sequence dimension
        done_tensor,  # Add batch and sequence dimension
...
def optimize_model(beta, policy_net, target_net, replay_buffer, optimizer, device, batch_size=32, gamma=0.99):
    ...
    # Sample method adjustments ensure these are already correctly shaped tensors
    actions = actions.to(device)
    rewards = rewards.to(device)
    dones = dones.to(device)
    weights = weights.to(device).unsqueeze(-1)  # Ensure weights have the correct shape

    # Ensure states and next_states are correctly formatted as per the updated sampling method
    states = states.to(device)
    next_states = next_states.to(device)

     # Process sequences through the networks and compute Q-values
    current_q_values = policy_net(states)
    # Adjust gather operation for sequence-aware action indexing
    current_q_values = current_q_values.gather(2, actions.unsqueeze(-1)).squeeze(-1)

    with torch.no_grad():
        next_q_values = target_net(next_states).max(2)[0]  # Max along actions dimension for each timestep
        next_q_values[dones] = 0.0  # Applying done mask; ensure dones is correctly broadcasted
    ...
    loss = loss.mean()  # Mean loss across the batch and sequence for backpropagation
