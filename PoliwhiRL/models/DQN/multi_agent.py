...
def gather_experiences(
    shared_model, config, state_shape, action_size, num_agents, num_episodes
):
    ...
    while total_episodes < num_episodes:
        with mp.Pool(processes=num_agents) as pool:
            ...
            results = pool.starmap(
                run_episode,
                [
                    (shared_model, config, state_shape, action_size, episode_num)
...
def train_step(shared_model, optimizer, shared_replay_buffer, config, device):
    ...
    td_errors = (
        torch.abs(current_q_values - target_q_values).mean(dim=1).detach().cpu().numpy()
    )
...
def setup_and_train_multi_agent(state_shape, action_size, config, num_agents=4):
    ...
    shared_replay_buffer = SequenceStorage(
        config["db_path"], config["replay_buffer_capacity"], config["sequence_length"]
    )
    shared_model, optimizer, device = init_shared_model(
        state_shape, action_size, config
    )

    for episode in range(2):
        experiences = gather_experiences(
            shared_model, config, state_shape, action_size, num_agents, 1
        )
