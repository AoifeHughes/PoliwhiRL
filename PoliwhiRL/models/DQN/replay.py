...
class PrioritizedReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done, lstm_state):
        self.episode_buffer.append(
            (state, action, reward, next_state, done, lstm_state)
        )
    ...
    def sample(self, batch_size):
        ...
        states = torch.FloatTensor(
            np.array([step[0] for seq in sequences for step in seq])
        )
        actions = torch.LongTensor(
            np.array([step[1] for seq in sequences for step in seq])
        )
        rewards = torch.FloatTensor(
            np.array([step[2] for seq in sequences for step in seq])
        )
        next_states = torch.FloatTensor(
            np.array([step[3] for seq in sequences for step in seq])
        )
        dones = torch.FloatTensor(
            np.array([step[4] for seq in sequences for step in seq])
        )

        initial_lstm_states = (
            torch.cat([s[0] for s in initial_lstm_states], dim=1),
        ...
        next_states = next_states.view(
            batch_size, self.sequence_length, *next_states.shape[1:]
        )
