...
class BaseDQNAgent:
    ...
    def update_model(self, batch):
        ...
        states = torch.tensor(np.transpose(np.stack(states), (0, 1, 4, 2, 3)), dtype=torch.float32).to(self.device)
        ...
        next_states = torch.tensor(np.transpose(np.stack(next_states), (0, 1, 4, 2, 3)), dtype=torch.float32).to(self.device)
