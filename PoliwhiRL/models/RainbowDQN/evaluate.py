...
def evaluate_model(config, env, policy_net):
    ...
    total_rewards = 0
    num_episodes = config.get(
        "eval_episodes", 10
    )  # Default to 10 episodes for evaluation if not specified.
    sequence_length = config.get(
        "sequence_length", 4
    )  # Default sequence length if not specified in config.
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            sequence_tensor = (
                torch.stack(state_sequence).unsqueeze(0).to(config["device"])
            )
