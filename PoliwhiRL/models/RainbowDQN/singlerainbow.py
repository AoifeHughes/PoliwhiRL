...
def run(config, env, policy_net, target_net, optimizer, replay_buffer):
    ...
    sequence_length = config.get("sequence_length", 10)  # Assuming a fixed sequence length
    ...
    for episode in tqdm(range(episodes, episodes + config["num_episodes"])):
        ...
        while not done:
            ...
            action, was_random = select_action_hybrid(state, policy_net, config, frame_idx, action_counts, num_actions, epsilon)
        ...
        for _ in range(config['opt_runs']):
            print(f"Optimizing model for episode {episode}")
