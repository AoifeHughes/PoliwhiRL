...
def run(config, env, policy_net, target_net, optimizer, replay_buffer):
    ...
    is_n_step = hasattr(replay_buffer, 'add_n_step_experience')  # Check if replay_buffer supports n-step
    ...
    for episode in tqdm(range(config.get("start_episode", 0), config.get("start_episode", 0) + config["num_episodes"])):
        ...
        while not done:
            ...
            if not config.get("eval_mode", False):
                if is_n_step:
                    # For n-step capable replay buffer, store experience differently
                    add_n_step_experience(state, action, reward, next_state, done, replay_buffer, n_step_buffer, config, config["gamma"])
                else:
                    # Original store_experience function for single-step replay buffer
