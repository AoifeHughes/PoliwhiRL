...
def worker(worker_id, batch_id, config, policy_net, target_net, frame_idx_start, action_counts, action_rewards):
    ...
    for episode in range(config["runs_per_worker"]):
        ...
        while not done:
            ...
            action, was_random = select_action_hybrid(state, policy_net, config, frame_idx, action_counts, len(action_counts), epsilon)
...
def run(config, policy_net, target_net, optimizer, replay_buffer, num_actions):
    ...
    for batch in tqdm(range(total_batches), desc="Batch Processing"):
        worker_args = [
            (i, batch, config, policy_net, target_net, frame_idx, action_counts.copy(), action_rewards.copy())
        ...
        for experiences, rewards, worker_action_counts, worker_action_rewards in worker_results:
