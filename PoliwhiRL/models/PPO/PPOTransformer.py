...
class ExplorationEncoder(nn.Module):
    def __init__(self, d_model):
        ...
        self.fc1 = nn.Linear(5, 16)  # 5 features per location: x, y, map_num, room, visits
...
class PPOTransformer(nn.Module):
    def __init__(self, input_shape, action_size, d_model=32, nhead=4, num_layers=2):
        ...
        self.fc_actor = nn.Linear(d_model * 2, action_size)  # *2 for concatenated exploration features
        self.fc_critic = nn.Linear(d_model * 2, 1)  # *2 for concatenated exploration features
    ...
    def forward(self, x, exploration_tensor=None):
        ...
        if exploration_tensor is not None:
            ...
            if len(exploration_tensor.shape) == 3 and exploration_tensor.shape[0] == batch_size:
