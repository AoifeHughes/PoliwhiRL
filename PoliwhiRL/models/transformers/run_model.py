...
class PokemonAgent:
    def __init__(self, state_dim, action_dim, config):
        ...
        # Initialize networks
        self.policy_net = PokemonTransformer(
            state_dim, action_dim, d_model, nhead, num_layers
        ).to(self.device)
        self.target_net = PokemonTransformer(
            state_dim, action_dim, d_model, nhead, num_layers
        ).to(self.device)
    ...
    def select_action(self, state):
        ...
        if random.random() > self.epsilon:
            with torch.no_grad():
                ...
                recent_states = [
                    s[0]
                    for s, _, _, _, _ in self.current_episode[
                ...
                padded_states = (
                    [first_state] * pad_length + recent_states + [state_tensor]
                )

                states = (
                    torch.cat(padded_states)
                    .view(self.sequence_length, 1, -1)
                    .to(self.device)
                )

                # Create an attention mask (False for real data, True for padding)
                attention_mask = torch.zeros(
                    1, self.sequence_length, dtype=torch.bool
                ).to(self.device)
    ...
    def optimize_model(self):
        ...
        state_batch = (
            torch.stack(
                [torch.cat([s for s, _, _, _, _ in episode]) for episode in batch]
            )
            .transpose(0, 1)
            .to(self.device)
        )
        action_batch = (
            torch.stack(
                [torch.cat([a for _, a, _, _, _ in episode]) for episode in batch]
            )
            .transpose(0, 1)
            .to(self.device)
        )
        reward_batch = (
            torch.stack(
                [torch.cat([r for _, _, r, _, _ in episode]) for episode in batch]
            )
            .transpose(0, 1)
            .to(self.device)
        )
        next_state_batch = (
            torch.stack(
                [torch.cat([ns for _, _, _, ns, _ in episode]) for episode in batch]
            )
            .transpose(0, 1)
            .to(self.device)
        )
        done_batch = (
            torch.stack(
                [torch.cat([d for _, _, _, _, d in episode]) for episode in batch]
            )
            .transpose(0, 1)
            .to(self.device)
        )
...
def train(env, agent, config):
    ...
    for episode in range(num_episodes):
        ...
        print(
            f"Episode {episode + 1}/{num_episodes}, Total Reward: {total_reward}, Epsilon: {agent.epsilon:.2f}"
        )
