...
def explore(num_episodes, rom_path, state_path, episode_length, sight=False, parallel=False):
    if parallel:
    ...
    env = environment(rom_path, state_path, episode_length, log_path=f"./logs/{savename}_log.json", use_sight=sight)
    for i in tqdm(range(num_episodes), desc="Episodes..."):
        ...
        while not done:
            ...
            print(f"Episode {i} | Reward {reward} | Steps {env.steps} | Current Timeout {env.timeout}", end="\r")
        env.reset()
    env.close()
