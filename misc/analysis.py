...
def generate_aggregated_rewards_plot(rewards_df):
    ...
    aggregated_rewards = rewards_df.groupby(['Run', 'Bin']).agg({'Reward': 'sum'}).reset_index()
...
def extract_data(json_files):
    ...
    rewards_data = []
    button_presses = []
    cumulative_entries = 0  # To keep track of the total number of entries processed so far

    for file_path in json_files:
        with open(file_path, 'r') as file:
            data = json.load(file)
            file_entries = len(data.keys()) - 1  # Subtracting one to ignore entry "1" (and "0" if present)
            for key in data:
            ...
            cumulative_entries += file_entries  # Update cumulative entries for next file
...
def generate_plots(runs_df, rewards_df, buttons_df):
    ...
    for metric in metrics:
        ...
        sns.boxplot(data=runs_df, x='Bin', y=metric, showfliers=False)  # Outliers not shown
    ...
    plt.figure(figsize=(12, 8))  # Adjusted for better visibility
    sns.boxplot(data=rewards_df, x='Bin', y='Reward', hue='Location', showfliers=False)  # Outliers not shown
    ...
    button_counts = buttons_df.groupby(['Bin', 'Button']).size().reset_index(name='Count')
    g = sns.FacetGrid(button_counts, col="Bin", col_wrap=4, height=4, sharex=True, aspect=2)  # Adjusted aspect for wider plots
    g.map_dataframe(lambda data, **kws: sns.barplot(data=data, x='Button', y='Count', **kws).set_xticklabels(data['Button'], rotation=45))
