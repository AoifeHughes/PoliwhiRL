...
class LearnGame:
    def __init__(self, rom_path, locations, location_address, device, SCALE_FACTOR, USE_GRAYSCALE, goal_loc):
    ...
    def select_action(self, state):
        if random.random() > self.epsilon:
            ...
            return torch.tensor([[random.randrange(len(self.movements))]], dtype=torch.long, device=self.device)  # Sending tensor to the specified device
    ...
    def optimize_model(self, batch_size=128):
        ...
        next_state_values = torch.zeros(batch_size, device=self.device)  # Tensor initialized on the specified device
        non_final_mask = torch.tensor(tuple(map(lambda s: s is not None, batch[3])), dtype=torch.bool, device=self.device)  # Tensor initialized on the specified device
