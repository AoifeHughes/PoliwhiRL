...
class DQN(nn.Module):
    def __init__(self, h, w, outputs, USE_GRAYSCALE):
        ...
        # Adjusted kernel sizes, strides, and added padding for more precise feature extraction
        self.conv1 = nn.Conv2d(1 if USE_GRAYSCALE else 3, 16, kernel_size=8, stride=4, padding=1)
        self.bn1 = nn.BatchNorm2d(16)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=4, stride=2, padding=1)
        self.bn2 = nn.BatchNorm2d(32)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.bn3 = nn.BatchNorm2d(64)
        # Removed the fourth convolutional layer to adapt to the smaller input size

        self._to_linear = None
        self._compute_conv_output_size(h, w)
        # Adjusted the size of the fully connected layer according to the new convolutional architecture
        self.fc1 = nn.Linear(self._to_linear, 512)  # Keeping a large FC layer for complex decision making
        self.fc2 = nn.Linear(512, outputs)

    def _compute_conv_output_size(self, h, w):
        # This function automatically computes the size of the linear layer inputs
