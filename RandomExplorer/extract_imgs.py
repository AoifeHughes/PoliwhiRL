...
def extract_images_by_map_num_loc(map_num_loc):
    ...
    for row_id, image_data, x, y, location in results:
        ...
        image_path = os.path.join(folder_name, f"image_row_id_{row_id}_x_{x}_y_{y}_location_{location}_map_num_loc_{map_num_loc}.png")
        image.save(image_path)
        print(f"Saved image {row_id} to {image_path}")
    
    conn.close()
    print(f"Extraction complete for map_num_loc {map_num_loc}. Images saved in folder: {folder_name}")
