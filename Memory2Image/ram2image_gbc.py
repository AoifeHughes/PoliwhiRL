...
class ImprovedGBCModel(nn.Module):
    def __init__(self):
        ...
        self.upsampler = nn.Upsample(size=(144, 160), mode='bilinear', align_corners=True)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        logger.log_epoch(epoch + 1, avg_train_loss, avg_val_loss, optimizer.param_groups[0]['lr'])
...
def setup_training(db_path):
    ...
    return model, train_loader, val_loader, test_loader, criterion, optimizer, scheduler, device
...
if __name__ == "__main__":
    ...
    model, train_loader, val_loader, test_loader, criterion, optimizer, scheduler, device = setup_training(db_path)
